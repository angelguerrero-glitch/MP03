...
public class Exercici5 {
    ...
    public static void main(String[] args) {
        ...
        do {            
            ...
            if(pes>0.0){
                cont++;
                //Es la primera pizza? cont==0
                if (cont==1) {
                    maxim=minim=pes; // Equival a fer maxim=pes; minim=pes;
                }else{
        ...
        if(cont==0) System.out.println("No s'ha  introduit cap  pizza en pes valid");
        else{        
        System.out.format("%s, s'han introduït %d pizzes %ni la suma dels pesos és %f %n", cadena, cont, suma);
        System.out.format(", la mitjana és %.3f, la mes pesda es %f i la menys %f %n", suma/cont,  maxim, minim);
        }
    }
